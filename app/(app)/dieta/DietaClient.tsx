...
import { PLANOS, faseFromDose, type FaseMounjaro } from "@/lib/diet-plans";
...
const CONFIANCA_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
...
function getFavoritos(): FavoritoRefeicao[] {
  if (typeof window === "undefined") return [];
  try { return JSON.parse(localStorage.getItem("momo-fav-refeicoes") || "[]"); }
  catch { return []; }
}

function saveFavoritos(favs: FavoritoRefeicao[]) {
                ...
                {refeicoesHoje.length === 0 ? (
                  ...
                  refeicoesHoje.map((r) => (
                    <RefeicaoCard
                      ...
                      onToggleFavorito={() => toggleFavorito(r)}
                      onRemover={() => removerRefeicao(r.id)}
                    />
                  ))
                )}
              </div>
