...
export default async function AdminFinanceiroPage() {
  ...
  const [
    ...
    admin.from("assinaturas").select("user_id, plano, status, valor, criado_em, proximo_vencimento, profiles!user_id(nome, email)").eq("status", "ativa").order("criado_em", { ascending: false }),
    admin.from("assinaturas").select("status, criado_em").order("criado_em", { ascending: true }),
  ]);

  // Build MRR por mês (últimos 12 meses)
  ...
  // Churn rate: canceladas no último mês / total
  ...
  const totalMesPassado = (todasAssinaturas || []).filter((a) => a.criado_em >= mesPassadoInicio && a.criado_em <= mesPassadoFim).length;
