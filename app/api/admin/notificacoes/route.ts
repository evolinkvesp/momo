...
export async function POST(req: Request) {
  ...
  if (!titulo || !mensagem || !segmento) {
    return NextResponse.json({ error: "titulo, mensagem e segmento são obrigatórios" }, { status: 400 });
  }
  ...
  if (segmento === "premium") {
    ...
    // Buscar users com dose recente nos últimos 10 dias
    ...
    if (idsComDose.length > 0) {
      query = query.not("id", "in", `(${idsComDose.join(",")})`);
    }
    ...
    query = query.in("plano_ativo", ["premium", "trial"]);
    if (idsComPeso.length > 0) {
      query = query.not("id", "in", `(${idsComPeso.join(",")})`);
    }
  } else {
    // ativos = premium + trial
    query = query.in("plano_ativo", ["premium", "trial"]);
  }

  const { data: usuarios } = await query;
  if (!usuarios || usuarios.length === 0) {
    return NextResponse.json({ ok: true, enviado: 0 });
  }

  const userIds = usuarios.map((u) => u.id);

  // Buscar todas as push subscriptions dos users
  ...
  if (!subs || subs.length === 0) {
    // Registrar no histórico mesmo sem subs
