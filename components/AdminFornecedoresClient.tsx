  ...
  async function handleAction(id: string, action: string, motivo?: string) {
    ...
    try {
      ...
      setFornecedores((prev) =>
        prev.map((f) => (f.id === id ? { ...f, status: data.novoStatus } : f))
      );

      const msgs: Record<string, string> = {
        aprovar:   "Fornecedor aprovado!",
        rejeitar:  "Fornecedor reprovado",
        suspender: "Fornecedor suspenso",
        reativar:  "Fornecedor reativado",
      };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {pendingCount > 0 && (
          <span className="a-badge-red text-[12px] py-1 px-3">
        ...
        {TABS.map((t) => {
          ...
          return (
            <button
              ...
              className={`a-tab ${tab === t.key ? "active" : ""} flex items-center gap-1.5`}
            >
              {t.label}
              {count > 0 && (
                <span
                  ...
                  style={{
                    background: t.key === "pendente" && count > 0 ? "rgba(239,68,68,0.2)" : "rgba(255,255,255,0.08)",
                    color: t.key === "pendente" && count > 0 ? "#ef4444" : "rgba(255,255,255,0.4)",
                  }}
                >
                  {count}
                </span>
      ...
      <div className="space-y-4">
        ...
        <AnimatePresence initial={false}>
          {filtered.map((f, i) => {
            ...
            const statusBadge =
              f.status === "ativo" ? "a-badge-green" :
              f.status === "pendente" ? "a-badge-yellow" :
              f.status === "suspenso" ? "a-badge-red" : "a-badge-gray";

            const statusLabel =
              f.status === "ativo" ? "ATIVO" :
              f.status === "pendente" ? "PENDENTE" :
              f.status === "suspenso" ? "SUSPENSO" : f.status.toUpperCase();
            ...
            return (
              <motion.div
                ...
                className="a-card-lg p-5 space-y-4"
              >
                {/* Row topo */}
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="text-[11px] text-[rgba(255,255,255,0.28)] mt-0.5">
                      Cadastrado em {format(new Date(f.created_at), "dd 'de' MMM 'de' yyyy", { locale: ptBR })}
                    </p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3 text-[12px]">
                  ...
                  {(f.endereco_cidade || f.endereco_estado) && (
                    <div className="flex items-center gap-1.5 text-[rgba(255,255,255,0.5)]">
                      <MapPin size={12} />
                      {[f.endereco_cidade, f.endereco_estado].filter(Boolean).join(", ")}
                    </div>
                  )}
                  {f.raio_entrega_km && (
                    <div className="flex items-center gap-1.5 text-[rgba(255,255,255,0.5)]">
                      <MapPin size={12} />
                      Raio {f.raio_entrega_km}km
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center gap-4 text-[12px]">
                  <span className="flex items-center gap-1.5 text-[rgba(255,255,255,0.5)]">
                    <Package size={12} />
                    {produtosAtivos.length} produto{produtosAtivos.length !== 1 ? "s" : ""}
                  </span>
                  {doses && (
                    <span className="text-[rgba(255,255,255,0.35)]">{doses}</span>
                  )}
                </div>
                ...
                {(f.avaliacao_media || f.total_pedidos) && (
                  <div className="flex items-center gap-4 text-[12px]">
                    ...
                    {f.total_pedidos !== null && (
                      <span className="text-[rgba(255,255,255,0.35)]">{f.total_pedidos} pedidos</span>
                    )}
                  </div>
                )}

                {/* Dados de análise para PENDENTES */}
                {f.status === "pendente" && (
                  <div className="border-t border-[rgba(255,255,255,0.06)] pt-4 space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-[12px]">
                      <div className="bg-[rgba(255,255,255,0.03)] rounded-xl px-3 py-2">
                        ...
                        <a
                          href={`https://www.receita.fazenda.gov.br/pessoajuridica/cnpj/cnpjreva/cnpjrevafrm.asp?cnpj=${f.cnpj.replace(/\D/g, "")}`}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-[#60a5fa] flex items-center gap-1 hover:underline font-mono"
                        >
                      ...
                      <div className="bg-[rgba(255,255,255,0.03)] rounded-xl px-3 py-2">
                        ...
                        <div className="flex gap-3">
                          <a href={`mailto:${f.email_contato}`} className="text-[#60a5fa] flex items-center gap-1 hover:underline">
                            <Mail size={10} /> Email
                          </a>
                          {(f.telefone || f.whatsapp) && (
                            <a href={`tel:${f.whatsapp || f.telefone}`} className="text-[#4ade80] flex items-center gap-1 hover:underline">
                              <Phone size={10} /> Tel
                            </a>
                          )}
                        </div>
                    ...
                    {f.fornecedor_produtos.length > 0 && (
                      <div className="bg-[rgba(255,255,255,0.03)] rounded-xl px-3 py-3">
                        ...
                        <div className="space-y-1.5">
                          {f.fornecedor_produtos.slice(0, 4).map((p) => (
                            <div key={p.id} className="flex items-center justify-between text-[12px]">
                              <span className="text-[rgba(255,255,255,0.55)]">
                                {TIPO_PRODUTO_LABEL[p.tipo_produto] || p.tipo_produto} · {p.dose_mg}mg
                              </span>
                          ...
                          {f.fornecedor_produtos.length > 4 && (
                            <p className="text-[rgba(255,255,255,0.25)] text-[11px]">
                              +{f.fornecedor_produtos.length - 4} mais...
                            </p>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex gap-3 pt-1">
                      <button
                        onClick={() => handleAction(f.id, "aprovar")}
                        disabled={!!isLoading}
                        className="a-btn-green flex items-center gap-2 flex-1 justify-center"
                      >
                        <Check size={14} />
                        {isLoading ? "Aprovando..." : "Aprovar fornecedor"}
                      </button>
                      <button
                        onClick={() => setRejectModal({ id: f.id, nome })}
                        disabled={!!isLoading}
                        className="a-btn-red flex items-center gap-2 flex-1 justify-center"
                      >
                        <X size={14} />
                        Reprovar
                      </button>
                ...
                {f.status === "ativo" && (
                  <div className="border-t border-[rgba(255,255,255,0.06)] pt-4">
                    <button
                      onClick={() => handleAction(f.id, "suspender")}
                      disabled={!!isLoading}
                      className="a-btn-red flex items-center gap-2 text-[12px] py-2"
                    >
                      <AlertTriangle size={13} />
                      {isLoading ? "Suspendendo..." : "Suspender fornecedor"}
                    </button>
                  </div>
                )}

                {/* Actions for SUSPENSO / REPROVADO */}
                {(f.status === "suspenso" || f.status === "reprovado") && (
                  <div className="border-t border-[rgba(255,255,255,0.06)] pt-4">
                    <button
                      onClick={() => handleAction(f.id, "reativar")}
                      disabled={!!isLoading}
                      className="a-btn-green flex items-center gap-2 text-[12px] py-2"
                    >
                      <RefreshCw size={13} />
                      {isLoading ? "Reativando..." : "Reativar fornecedor"}
                    </button>
                  </div>
                )}
              </motion.div>
      ...
      <AnimatePresence>
        {rejectModal && (
          <motion.div
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="fixed inset-0 z-50 flex items-end md:items-center justify-center p-4 bg-[rgba(0,0,0,0.7)]"
          ...
          >
            <motion.div
              ...
              className="a-card-lg p-6 w-full max-w-md space-y-4"
            >
              <div>
                ...
                <p className="text-[13px] text-[rgba(255,255,255,0.4)] mt-0.5">
                  Informe o motivo da reprovação para o fornecedor.
                </p>
              </div>
              ...
              <textarea
                ...
                placeholder="Ex: CNPJ inválido, documentação incompleta, fora da área de cobertura..."
                className="a-input resize-none h-28"
              />

              <div className="flex gap-3">
                <button onClick={() => setRejectModal(null)} className="a-btn-ghost flex-1">
                  Cancelar
                </button>
                <button
                  onClick={confirmReject}
                  disabled={!rejectMotivo.trim()}
                  className="a-btn-red flex-1"
                >
