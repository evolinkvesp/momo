...
export function AdminUserDetailClient({
  ...
  const pesoData = [...medicoes]
    .filter((m) => m.peso_kg)
    .reverse()
    .map((m) => ({ date: format(new Date(m.data_medicao), "dd/MM"), value: m.peso_kg }));
  ...
  return (
    <div className="space-y-6">
      ...
      <Link
        href="/admin/usuarios"
        className="flex items-center gap-2 text-[rgba(255,255,255,0.4)] hover:text-white text-[13px] font-semibold transition-colors"
      >
        <ArrowLeft size={15} />
          ...
          <div className="flex-1">
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              ...
              <span className="text-[11px] text-[rgba(255,255,255,0.3)]">
                Desde {format(new Date(usuario.created_at), "dd/MM/yyyy")}
              </span>
            </div>
          </div>
        </div>
      ...
      {pesoData.length > 1 && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="a-card p-5">
          <div className="flex items-center gap-2 mb-4">
            <Weight size={14} className="text-[rgba(255,255,255,0.3)]" />
            <h4 className="text-[13px] font-bold text-white">Evolução do peso</h4>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={pesoData}>
                ...
                <Tooltip
                  ...
                  contentStyle={{ backgroundColor: "#1a1a1a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "10px", fontSize: "11px", color: "#fff" }}
      ...
      {doses.length > 0 && (
        <div className="a-card-lg p-5">
          <h4 className="text-[13px] font-bold text-white mb-4 flex items-center gap-2">
            <Syringe size={14} className="text-[rgba(255,255,255,0.3)]" />
            Últimas doses
          </h4>
          <div className="space-y-2">
            {doses.map((d) => (
              <div key={d.id} className="flex items-center justify-between py-2 border-b border-[rgba(255,255,255,0.05)] last:border-0">
                <div>
                  <p className="text-[13px] font-semibold text-white">{d.dose_mg} mg</p>
                  <p className="text-[11px] text-[rgba(255,255,255,0.35)]">
                    {d.local_aplicacao || d.lado_corpo || "–"}
                  </p>
                </div>
                <p className="text-[11px] text-[rgba(255,255,255,0.3)]">
                  {format(new Date(d.data_aplicacao), "dd/MM/yyyy")}
                </p>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {assinaturas.map((a, i) => (
              <div key={i} className="flex items-center justify-between py-2 border-b border-[rgba(255,255,255,0.05)] last:border-0">
                <div>
                  <p className="text-[13px] font-semibold text-white capitalize">{a.plano}</p>
                  <p className="text-[11px] text-[rgba(255,255,255,0.35)]">
                    Desde {format(new Date(a.criado_em), "dd/MM/yyyy")}
                  </p>
                </div>
                <div className="text-right">
                  {a.valor && <p className="text-[13px] font-bold text-white">{formatBRL(a.valor)}</p>}
                  <span className={a.status === "ativa" ? "a-badge-green" : a.status === "cancelada" ? "a-badge-red" : "a-badge-gray"}>
      ...
      {pedidos.length > 0 && (
        <div className="a-card-lg p-5">
          <h4 className="text-[13px] font-bold text-white mb-4 flex items-center gap-2">
            <ShoppingBag size={14} className="text-[rgba(255,255,255,0.3)]" />
            Pedidos
          </h4>
          <div className="space-y-2">
            {pedidos.map((p) => {
              const color = STATUS_PEDIDO_COLOR[p.status] || "#ffffff";
              return (
                <div key={p.id} className="flex items-center justify-between py-2 border-b border-[rgba(255,255,255,0.05)] last:border-0">
                  <div>
                    <p className="text-[13px] font-semibold text-white">{p.codigo}</p>
                    <p className="text-[11px] text-[rgba(255,255,255,0.35)]">
      ...
      {avaliacoes.length > 0 && (
        <div className="a-card-lg p-5">
          <h4 className="text-[13px] font-bold text-white mb-4 flex items-center gap-2">
            <Star size={14} className="text-[rgba(255,255,255,0.3)]" />
            Avaliações deixadas
          </h4>
                ...
                <div className="flex items-center gap-1 mb-1">
                  {Array.from({ length: 5 }).map((_, idx) => (
                    <Star
                      key={idx}
                      size={11}
                      fill={idx < a.nota ? "#fbbf24" : "transparent"}
                      stroke={idx < a.nota ? "#fbbf24" : "rgba(255,255,255,0.2)"}
                    />
                  ))}
                  <span className="text-[10px] text-[rgba(255,255,255,0.3)] ml-1">
