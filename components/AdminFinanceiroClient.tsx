...
export function AdminFinanceiroClient({
  ...
  premiumCount: number;
  assinaturasAtivas: any[];
  mrrData: { mes: string; mrr: number }[];
  churnRate: string;
  ...
  const ltv = premiumCount > 0 ? totalRecebido / Math.max(1, premiumCount) : 0;

  const metricas = [
    { label: "MRR",             value: formatBRL(mrr),         sub: `${premiumCount} assinantes × ${formatBRL(PRECO_PLANO)}`,  icon: DollarSign, color: "#4ade80" },
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {metricas.map((m, i) => (
          ...
          >
            <div
              className="h-[30px] w-[30px] rounded-[10px] flex items-center justify-center mb-3"
              ...
              <thead>
                <tr className="border-b border-[rgba(255,255,255,0.06)]">
                  {["Usuário", "Plano", "Valor", "Desde", "Próx. vencto", "Status"].map((h) => (
                    <th key={h} className="text-left text-[10px] font-bold text-[rgba(255,255,255,0.28)] uppercase tracking-wider pb-3 px-1">
                      {h}
                    </th>
                  ))}
                </tr>
                ...
                {assinaturasAtivas.map((a, i) => (
                  <tr key={i} className="border-b border-[rgba(255,255,255,0.04)] hover:bg-[rgba(255,255,255,0.02)] transition-colors">
                    <td className="py-3 px-1">
                      <p className="text-white font-semibold truncate max-w-[140px]">{a.profiles?.nome || "–"}</p>
                      <p className="text-[rgba(255,255,255,0.3)] truncate max-w-[140px]">{a.profiles?.email || "–"}</p>
                    </td>
                    ...
                    <td className="py-3 px-1 text-[rgba(255,255,255,0.4)]">
                      {a.criado_em ? format(new Date(a.criado_em), "dd/MM/yyyy") : "–"}
                    </td>
                    <td className="py-3 px-1 text-[rgba(255,255,255,0.4)]">
                      {a.proximo_vencimento ? format(new Date(a.proximo_vencimento), "dd/MM/yyyy") : "–"}
                    </td>
                    <td className="py-3 px-1">
