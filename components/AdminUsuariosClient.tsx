...
import { Search, Users, MoreVertical, ChevronRight, Crown, RotateCcw, Shield, ShieldOff } from "lucide-react";
...
const FILTROS = [
  { key: "todos", label: "Todos" }, { key: "premium", label: "Premium" },
  { key: "trial", label: "Trial" }, { key: "expirado", label: "Expirado" }, { key: "bloqueado", label: "Bloqueado" },
];
    ...
    <div className="space-y-6" onClick={() => setActionMenu(null)}>
      ...
      <div className="space-y-2">
        ...
        {filtered.map((u, i) => {
          ...
          return (
            <motion.div key={u.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.03, 0.3) }} className="a-card px-4 py-3 flex items-center gap-3">
              ...
              <div className="hidden md:block text-right shrink-0">
                ...
                {u.ultima_dose && <p className="text-[10px] text-[rgba(255,255,255,0.2)]">Dose: {formatDistanceToNow(new Date(u.ultima_dose), { locale: ptBR, addSuffix: true })}</p>}
              </div>
