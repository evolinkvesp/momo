...
const FILTROS = [
  ...
  { key: "problema",   label: "Com problema" },
...
export function AdminPedidosClient({ pedidos: initial }: { pedidos: Pedido[] }) {
  ...
  const filtered = pedidos.filter((p) => {
    ...
    if (filtro === "problema") return ["cancelado"].includes(p.status) || !!p.cancelamento_motivo || !!p.observacoes_paciente;
    return true;
  });
  ...
  async function handleAction(id: string, action: string, mensagem?: string) {
    ...
    try {
      const res = await fetch(`/api/admin/pedidos/${id}`, {
        ...
        body: JSON.stringify({ action, mensagem }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);

      setPedidos((prev) =>
        prev.map((p) => p.id === id ? { ...p, status: data.novoStatus } : p)
      );

      const msgs: Record<string, string> = {
        reembolso:      "Reembolso registrado",
        forcar_entregue: "Marcado como entregue",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2">
        {FILTROS.map((f) => (
          <button key={f.key} onClick={() => setFiltro(f.key)} className={`a-tab ${filtro === f.key ? "active" : ""}`}>
            {f.label}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-4">
        ...
        {filtered.map((p, i) => {
          ...
          const prodLabel = p.produto
            ? `${TIPO_PRODUTO_LABEL[p.produto.tipo_produto] || p.produto.tipo_produto} · ${p.produto.dose_mg}mg`
            : "–";
            ...
            >
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-[11px] text-[rgba(255,255,255,0.35)] mt-0.5">
                    {format(new Date(p.created_at), "dd 'de' MMM yyyy, HH:mm", { locale: ptBR })}
                  </p>
                </div>
                <span
              ...
              {(p.cancelamento_motivo || p.observacoes_paciente) && (
                <div className="bg-[rgba(239,68,68,0.06)] border border-[rgba(239,68,68,0.12)] rounded-xl p-3">
                  <p className="text-[11px] font-bold text-[#ef4444] uppercase mb-1">
                    {p.cancelamento_motivo ? "Motivo do cancelamento" : "Observação do paciente"}
                  </p>
              ...
              <div className="flex flex-wrap gap-2 pt-1">
                <button
                  onClick={() => handleAction(p.id, "forcar_entregue")}
                  disabled={!!isLoading || p.status === "entregue"}
                  className="a-btn-green text-[12px] py-2 flex items-center gap-1.5"
                >
                  <Check size={13} /> Forçar entregue
                </button>
                <button
                  onClick={() => handleAction(p.id, "reembolso")}
                  disabled={!!isLoading}
                  className="a-btn-red text-[12px] py-2 flex items-center gap-1.5"
                >
                  <RefreshCw size={13} /> Reembolso
                </button>
                <button
                  onClick={() => setMediacao({ id: p.id, codigo: p.codigo })}
                  disabled={!!isLoading}
                  className="a-btn-ghost text-[12px] py-2 flex items-center gap-1.5"
                >
                  <AlertTriangle size={13} /> Mediar
                </button>
                {p.paciente?.email && (
                  <a
                    href={`mailto:${p.paciente.email}?subject=Pedido ${p.codigo}`}
                    className="a-btn-ghost text-[12px] py-2 flex items-center gap-1.5"
                  >
                    <Mail size={13} /> Paciente
                  </a>
                )}
                {p.fornecedor?.email_contato && (
                  <a
                    href={`mailto:${p.fornecedor.email_contato}?subject=Pedido ${p.codigo}`}
                    className="a-btn-ghost text-[12px] py-2 flex items-center gap-1.5"
                  >
                    <Mail size={13} /> Fornecedor
                  </a>
                )}
              </div>
            </motion.div>
          ...
          >
            ...
            >
              ...
              <p className="text-[13px] text-[rgba(255,255,255,0.4)]">
                Esta mensagem será registrada no log admin.
              </p>
              <textarea
                value={mensagemMediacao}
                onChange={(e) => setMensagemMediacao(e.target.value)}
                placeholder="Descreva a decisão ou instrução de mediação..."
              ...
              <div className="flex gap-3">
                ...
                <button
                  onClick={async () => {
                    ...
                    await fetch(`/api/admin/pedidos/${mediacao.id}`, {
                      method: "PATCH",
                      headers: { "Content-Type": "application/json" },
                      body: JSON.stringify({ action: "cancelar", mensagem: mensagemMediacao }),
                    });
                    toast.success("Mediação registrada");
                    setMediacao(null);
                    setMensagemMediacao("");
                  }}
                  disabled={!mensagemMediacao.trim()}
                  className="a-btn-blue flex-1"
                >
                  Registrar mediação
                </button>
              </div>
